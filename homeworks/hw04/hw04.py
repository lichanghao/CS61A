...
def replace_leaf(t, old, new):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert is_tree(t), "The input must be tree"
    p = t
    new_label = label(p)
    new_branches = []
    if is_leaf(p) and label(p) == old:
        return tree(new)
    else:
        for branch in branches(p):
            new_branches.append(replace_leaf(branch, old, new))
...
def prune_leaves(t, vals):
    ...
    for branch in branches(t):
        if replace_leaf(branch, old, new) != None:
            new_branches.append(prune_leaves(branch, vals))
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        if length(right(m)) * total_weight(end(right(m))) == length(left(m)) * total_weight(end(left(m))):
